...
Page({
  ...
  backSetData:function(city,code){
    ...
    prevPage.setData({ cityAdcode: [city, code] })
    prevPage.setFromCity(true)
  ...
  backIndex: function(e){
    ...
    if(this.options.fromMap){//来自地图
      let pages = getCurrentPages();
      let prevPage = pages[pages.length - 2]
      let addrObj = prevPage.data.addrObj
      let addrTemp = JSON.parse(this.options.addrTemp)
      addrObj.consignee = addrTemp.consignee
      addrObj.consigneeMobile = addrTemp.consigneeMobile
      addrObj.addressDetail = addrTemp.addressDetail
      prevPage.setData({ adcode: e.currentTarget.dataset.code, addrObj })
      wx.navigateBack({ delta: 1 })
    } else {
  ...
  selectCity :function(e) {
    ...
    if (this.options.fromMap) {//来自地图
      ...
      addrObj.consigneeMobile = addrTemp.consigneeMobile
      addrObj.addressDetail = addrTemp.addressDetail
      prevPage.setData({ adcode: adcode, addrObj })
      wx.navigateBack({ delta: 1 })
    } else{
