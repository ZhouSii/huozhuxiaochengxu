...
Page({
  ...
  successSubmit: function () {
    if (this.data.remark === '') {
      prevPage.setData({ 'orderNextform.remark': '' })
      ...
      if (reg.test(this.data.remark)) {
        ...
        prevPage.setData({ 'orderNextform.remark': this.data.remark })
        wx.navigateBack({ delta: 1 })
      } else {
        wx.showToast({
          title: '只能输入汉字、英文、数字，。',
  ...
  onLoad: function (options) {
    ...
    this.setData({ remark: prevPage.data.orderNextform.remark })
    if (app.storage.get_s('userInfo')){
      this.setData({ mobile: app.storage.get_s('userInfo').mobile })
    }
